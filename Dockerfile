...
RUN echo "install maven and gradle" \
    && wget -q https://services.gradle.org/distributions/gradle-5.6.3-bin.zip -O /tmp/gradle.zip \
    && mkdir /opt/gradle \
...
RUN echo "install IntelliJ IDEA" \
    && wget -q https://download.jetbrains.com/idea/ideaIC-2019.2.3-no-jbr.tar.gz -O /tmp/idea.tar.gz \
    && mkdir -p /opt/idea \
    && tar zxvf /tmp/idea.tar.gz --strip-components=1 -C /opt/idea

ENV PATH="/opt/gradle/gradle-5.6.3/bin:${PATH}"
