...
RUN echo "install AdoptOpenJDK 11" \
    ...
    && apt install -y ./adoptopenjdk-11-hotspot_11.0.3+7-1_amd64.deb\
    && rm ./adoptopenjdk-11-hotspot_11.0.3+7-1_amd64.deb

RUN echo "install maven and gradle" \
    && wget -q https://services.gradle.org/distributions/gradle-5.5.1-bin.zip -O /tmp/gradle.zip \
    && mkdir /opt/gradle \
...
RUN echo "install IntelliJ IDEA" \
    && wget -q https://download.jetbrains.com/idea/ideaIC-2019.1.3-no-jbr.tar.gz -O /tmp/idea.tar.gz \
    && mkdir -p /opt/idea \
